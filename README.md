...
2. Install the package. For example:
   ...
   ```shell
   sudo dpkg -i zsh-patina_1.10.0_amd64.deb
   ```
...
2. Extract the archive to an arbitrary directory. For example, if you want to extract it to `~/.zsh-patina`:
   ...
   ```shell
   mkdir ~/.zsh-patina
   tar xfz zsh-patina-v1.10.0-aarch64-apple-darwin.tar.gz -C ~/.zsh-patina --strip-components 1
   ```
...
| `catppuccin-frappe` | A soothing dark theme with muted tones. Based on [Catppuccin Frappé](https://catppuccin.com/palette#flavor-frappe). |
| `catppuccin-latte` | A soothing light theme with pastel tones. Based on [Catppuccin Latte](https://catppuccin.com/palette#flavor-latte). |
